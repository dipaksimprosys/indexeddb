...
function renderHtml(data, filterText = "") {
    ...
    const filteredData = filterText ? data.filter((user)=>String(user.userid).includes(filterText) || user.name.toLowerCase().includes(filterText.toLowerCase()) || user.email.toLowerCase().includes(filterText.toLowerCase())) : data;
    document.getElementById("user_data").innerHTML = filteredData.map((user)=>`<tr>
      <td>${user.userid}</td> 
      <td>${user.name}</td>
      <td>${user.email}</td> 
      <td><button class="remove-button" data-id="${user.userid}">Remove</button></td>
    </tr>`).join("");
    document.querySelectorAll(".remove-button").forEach((el)=>el.addEventListener("click", async ()=>{
...
database.openDatabase("mydb").then((db)=>database.viewAllUsers()).then((data)=>{
    renderHtml(data);
});
